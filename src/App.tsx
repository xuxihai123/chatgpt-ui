...
import { StreamToEventSource } from '@/utils/app/stream2sse';
...
const firstMsg = { role: 'system', content: 'You are ChatGPT, a large language model trained by OpenAI. Answer as concisely as possible.\\nKnowledge cutoff: 2021-09-01\\nCurrent date: 2023-08-22' };
...
export default function Home() {
    ...
    const handleSend = async (message: Message, isResend: boolean) => {
        if (selectedConversation) {
            ...
            const modelObj = updatedConversation.model;
            ...
            const chatPayload: any = {
                ...
                max_tokens: 1000,
                model: modelObj.id,
                messages: [firstMsg, ...updatedConversation.messages],
                stream: true
            };
            console.log(chatPayload);

            const controller = new AbortController();
            const response = await fetch(BASE_URI+'/chat/completions', {
                method: 'POST',
                headers: {
            ...
            const stream: any = StreamToEventSource(response);
    ...
    const fetchModels = async (key: string) => {
        // console.log(key);
        const response = await fetch(BASE_URI+'/models', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
        ...
        respdata.data.forEach((temp: any) => {
            temp.name = temp.id;
        });

        setModels(respdata.data);
        setModelError(false);
    };
